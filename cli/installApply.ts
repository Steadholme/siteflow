...
import type { SiteFlowCommandRunner } from "./doctor.js";
import type { RenderedInstallAsset } from "./installAssets.js";
import type { InstallPlan } from "./installPlan.js";

export type InstallApplyStepStatus = "created" | "updated" | "skipped" | "validated" | "failed";
export type InstallLinkStrategy = "symlink" | "copy";
type FetchLike = (input: string | URL, init?: RequestInit) => Promise<Response>;

export interface InstallApplyStep {
  id: string;
  status: InstallApplyStepStatus;
  summary: string;
  path?: string;
}
...
export interface InstallApplyResult {
  ...
  doctor: DoctorReport;
  router: InstallPlan["installState"]["router"];
}

export interface InstallApplyOptions {
  root?: string;
  runner?: SiteFlowCommandRunner;
  linkStrategy?: InstallLinkStrategy;
  reloadNginx?: boolean;
...
const defaultRunner: SiteFlowCommandRunner = async () => ({
  exitCode: 0,
  stdout: "",
  stderr: ""
});

function mapPath(filePath: string, root?: string) {
...
async function tryReadText(filePath: string) {
...
async function writeTextAsset(asset: RenderedInstallAsset, root: string | undefined, stepId: string): Promise<InstallApplyStep> {
...
async function writeSecretFile(secret: InstallPlan["secrets"][number], root: string | undefined): Promise<InstallApplyStep> {
...
async function restorePath(filePath: string, snapshot: PathSnapshot) {
...
async function writeNginxEnabled(
  enabledPath: string,
...
function passCheck(id: string, label: string, summary: string): DoctorCheck {
  return {
    id,
    label,
    status: "pass",
...
function failCheck(id: string, label: string, summary: string, remediation?: string): DoctorCheck {
  return {
    id,
    label,
    status: "fail",
...
async function checkTextAsset(asset: RenderedInstallAsset, root: string | undefined, id: string, label: string): Promise<DoctorCheck> {
...
function apiPort(plan: InstallPlan) {
...
async function waitForApiHealth(plan: InstallPlan, options: InstallApplyOptions): Promise<InstallApplyStep> {
...
async function runInstallDoctor(plan: InstallPlan, options: InstallApplyOptions): Promise<DoctorReport> {
  ...
  if (plan.renderedAssets.nginx) {
    ...
    checks.push(
      activeConfig === plan.renderedAssets.nginx.content
...
function doctorStep(report: DoctorReport): InstallApplyStep {
...
async function ensureDirectories(plan: InstallPlan, root: string | undefined): Promise<InstallApplyStep> {
  const directories = [
...
async function applyServiceAssets(plan: InstallPlan, options: Required<Pick<InstallApplyOptions, "startServices">> & InstallApplyOptions) {
...
async function applyNginx(plan: InstallPlan, options: Required<Pick<InstallApplyOptions, "linkStrategy" | "reloadNginx">> & InstallApplyOptions) {
...
export async function applyInstallPlan(plan: InstallPlan, options: InstallApplyOptions = {}): Promise<InstallApplyResult> {
  const steps: InstallApplyStep[] = [];
  ...
  const doctor = options.runFinalDoctor === false ? { status: "pass" as const, checks: [] } : await runInstallDoctor(plan, { ...options, startServices });
