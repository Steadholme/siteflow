...
describe("install asset renderers", () => {
  ...
  it("renders SITEFLOW_BASE_DOMAIN into the non-secret env file", () => {
    ...
    expect(env.content).toContain("SITEFLOW_TRUST_PROXY=loopback");
  ...
  it("renders a single-host Compose file with secret file references", () => {
    ...
    expect(compose.content).toContain('    user: "${SITEFLOW_WORKER_USER:-0:0}"');
    expect(compose.content).toContain("    group_add:");
    expect(compose.content).toContain('      - "${SITEFLOW_DOCKER_SOCKET_GID:-0}"');
    ...
    expect(compose.content).toContain('SITEFLOW_TRUST_PROXY: "loopback"');
