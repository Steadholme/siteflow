...
import { assertReleaseChannel, SiteFlowNotFoundError } from "./readRepository.js";
const functionConcurrency = new Map();
...
function setCorsHeaders(response, allowedOrigin, options) {
...
async function loadFunctionModule(functionPath) {
    return nativeImport(pathToFileURL(functionPath).href);
}
function sendJson(response, statusCode, body, allowedOrigin, method) {
...
function requiredIntegerParam(url, name, min, max) {
...
function optionalIntegerParam(url, name, fallback, min, max) {
    const value = url.searchParams.get(name);
    if (!value) {
        return fallback;
    }
...
function logQueryFromUrl(projectId, url) {
    return {
        projectId,
...
function notFound(response, allowedOrigin, method) {
...
function lowerCaseRequestHeaders(request) {
...
async function deliverLogDrain(plan, command, options) {
...
function actorFromGitHub(payload, fallbackName) {
...
function repositoryFromGitHub(payload) {
...
function sanitizeGitHubRepositoryPayload(payload) {
    const repository = recordField(payload, "repository");
    return {
        id: numberField(repository, "id"),
...
function normalizeGitHubPush(payload, deliveryId, receivedAt) {
...
function normalizeGitHubPullRequest(payload, deliveryId, receivedAt) {
...
async function authorizeMutation(request, response, options, permission, projectId) {
...
async function authenticatedPermissions(request, options, projectId) {
    const token = bearerToken(request);
    if (token && token === options.apiToken) {
...
function staticAssetCacheControl(filePath) {
    const extension = path.extname(filePath).toLowerCase();
...
function weakStaticEtag(route, resolvedPath, body) {
...
function ifNoneMatchValues(value) {
...
function weakEtagMatches(candidate, etag) {
    const normalize = (value) => value.startsWith("W/") ? value.slice(2) : value;
    return normalize(candidate) === normalize(etag);
}
function strongEtagMatches(candidate, etag) {
    return !candidate.startsWith("W/") && !etag.startsWith("W/") && candidate === etag;
}
function requestIfMatchPasses(request, etag) {
...
function requestHasFreshModifiedSince(request, modifiedAt) {
...
function requestIfUnmodifiedSincePasses(request, modifiedAt) {
...
function requestIfRangeMatches(request, etag, modifiedAt) {
...
function acceptedEncodingQuality(value, encoding) {
...
async function selectEncodedArtifactFile(request, filePath) {
    ...
    const candidates = encodedArtifacts
        .map((candidate) => ({
        ...candidate,
...
function imageContentType(format, sourceContentType) {
    if (format === "auto") {
...
function assertSafeImageSource(value) {
...
function safeImageArtifactPath(source) {
...
async function readArtifactImage(route, source) {
...
function assertImageWidthAllowed(width, config) {
...
function assertImageQualityAllowed(quality, config) {
...
function cacheControlForImage(config) {
    const ttl = config?.minimumCacheTTL;
    return ttl === undefined
        ? "public, max-age=31536000, immutable"
        : `public, max-age=${ttl}`;
}
async function readBlobImage(repository, route, source) {
    if (!route.projectId) {
...
function functionForPath(functions, pathname) {
...
function safeFunctionPath(route, sourcePath) {
    const root = path.resolve(route.artifactRoot);
...
function safeRoutePath(urlPath, entrypoint) {
...
function cleanUrlCandidate(routePath) {
...
async function tryResolveArtifactPath(root, relativePath) {
...
async function resolveArtifactFile(route, urlPath) {
    const root = path.resolve(route.artifactRoot);
...
function redactionPatternsFor(values) {
    return Object.values(values ?? {})
        .filter((value) => value.length >= 4)
        .map((value) => new RegExp(escapeRegExp(value), "g"));
}
function captureLogArg(value, secretPatterns) {
...
async function withRuntimeEnvironment(environment, callback) {
...
async function captureFunctionLogs(callback, secretPatterns = []) {
    const logs = [];
...
function functionConcurrencyKey(route, entry) {
    return `${route.deploymentId}:${entry.path}`;
}
function memoryLimitExceeded(entry) {
    const memoryMb = entry.memoryMb ?? 512;
    const rssMb = process.memoryUsage().rss / (1024 * 1024);
    return rssMb > memoryMb;
}
function currentFunctionConcurrency(route, entry) {
    return functionConcurrency.get(functionConcurrencyKey(route, entry)) ?? 0;
}
function enterFunctionInvocation(route, entry) {
...
function headerValueFromRule(value, pathname) {
...
function mergeCommaHeader(existingValue, nextValue) {
...
function applyRoutingHeaders(response, rules, pathname) {
...
function redirectLocation(rule, pathname) {
    return applyRoutingDestination(pathname, rule.source, rule.destination) ?? pathname;
}
function appendSearchToRedirectLocation(location, search) {
    if (!search) {
        return location;
    }
...
function routingRedirectLocation(rule, pathname, url) {
...
function matchArtifactRoutingRules(route, pathname) {
...
function canonicalStaticLocation(pathname, route, resolved) {
...
function staticCanonicalRedirectLocation(location, url) {
    return appendSearchToRedirectLocation(location, url.search);
}
function redirectStaticCanonical(response, location, url, allowedOrigin) {
...
function setDefaultStaticSecurityHeaders(response) {
    response.setHeader("x-content-type-options", "nosniff");
    response.setHeader("referrer-policy", "strict-origin-when-cross-origin");
}
function endResponseWithoutBody(response, statusCode) {
    response.statusCode = statusCode;
    response.removeHeader("content-length");
    response.end();
}
function setRoutingResponseHeaders(response, match, pathname) {
    if (!match) {
        return;
    }
...
function runtimeSetCookieHeaders(headers) {
    const readableHeaders = headers;
    return typeof readableHeaders.getSetCookie === "function" ? readableHeaders.getSetCookie() : [];
}
function setRuntimeResponseHeaders(response, headers) {
...
async function invokeFunctionRoute(context, route, entry, options, routingMatch) {
    ...
    try {
        ...
        const captured = await captureFunctionLogs(() => withTimeout(withRuntimeEnvironment(runtimeEnvironment, async () => runtimeResultToResponse(await handler(runtimeRequest, {
            params: {},
            path: url.pathname,
            requestId,
            deploymentId: route.deploymentId,
            env: runtimeEnvironment
        }))), timeoutMs), secretPatterns);
...
async function tryServeArtifactRoute(context, options) {
    const { request, response, url } = context;
    ...
    let routingMatch = matchArtifactRoutingRules(route, url.pathname);
...
async function tryServeImageOptimizationRoute(context, options) {
    const { request, response, url } = context;
...
async function handleApiRoute(context, options) {
    ...
    if ((request.method === "GET" || request.method === "HEAD") && segments.length === 3 && segments[0] === "api" && segments[1] === "auth" && segments[2] === "verify") {
        ...
        sendJson(response, 200, { authenticated: true, authRequired: Boolean(options.apiToken), baseDomain: options.baseDomain }, options.allowedOrigin, request.method);
        return;
    }
    if (segments[0] !== "api") {
    ...
    if (request.method === "POST" && segments.length === 4 && segments[1] === "webhooks" && segments[2] === "git" && segments[3] === "github") {
        ...
        const payload = rawBody.toString("utf8").trim() ? JSON.parse(rawBody.toString("utf8")) : {};
    ...
    if (request.method === "POST" && segments.length === 2 && segments[1] === "projects") {
        ...
        sendJson(response, 201, await repository.createProject((await readJsonBody(request))), options.allowedOrigin);
        return;
    }
    if (segments[1] === "projects" && segments[2]) {
        ...
        if (request.method === "PATCH" && segments.length === 3) {
            ...
            sendJson(response, 200, await repository.updateProject(projectId, (await readJsonBody(request))), options.allowedOrigin);
            return;
        }
        ...
        if ((request.method === "GET" || request.method === "HEAD") && segments.length === 4 && segments[3] === "settings") {
            ...
            }, options.allowedOrigin, request.method);
            return;
        }
        ...
        if (request.method === "POST" && segments.length === 5 && segments[3] === "analytics" && segments[4] === "events") {
            sendJson(response, 202, await repository.ingestAnalyticsEvent({
        ...
        if ((request.method === "GET" || request.method === "HEAD") && segments.length === 4 && segments[3] === "logs") {
            sendJson(response, 200, await repository.queryLogs(logQueryFromUrl(projectId, url)), options.allowedOrigin, request.method);
            return;
        }
        ...
        if (request.method === "POST" && segments.length === 4 && segments[3] === "log-queries") {
            ...
            sendJson(response, 201, await repository.saveLogQuery({
                ...(await readJsonBody(request)),
                projectId
            }), options.allowedOrigin);
            return;
        ...
        if (request.method === "POST" && segments.length === 4 && segments[3] === "log-drains") {
            ...
            sendJson(response, 201, await repository.createLogDrain({
                ...(await readJsonBody(request)),
                projectId
            }), options.allowedOrigin);
            return;
        ...
        if (request.method === "POST" && segments.length === 4 && segments[3] === "environment-variables") {
            ...
            sendJson(response, 200, await repository.upsertEnvironmentVariable({
                ...(await readJsonBody(request)),
                projectId
            }), options.allowedOrigin);
            return;
        ...
        if (request.method === "POST" && segments.length === 4 && segments[3] === "team-members") {
            ...
            sendJson(response, 200, await repository.upsertTeamMember({
                ...(await readJsonBody(request)),
                projectId
            }), options.allowedOrigin);
            return;
        ...
        if (request.method === "DELETE" && segments.length === 5 && segments[3] === "team-members") {
            ...
            sendJson(response, 200, await repository.removeTeamMember({
                ...(await readJsonBody(request)),
                projectId,
        ...
        if (request.method === "POST" && segments.length === 4 && segments[3] === "api-tokens") {
            ...
            sendJson(response, 201, await repository.createApiToken({
                ...(await readJsonBody(request)),
                projectId
            }), options.allowedOrigin);
            return;
        ...
        if (request.method === "DELETE" && segments.length === 5 && segments[3] === "api-tokens") {
            ...
            sendJson(response, 200, await repository.revokeApiToken({
                ...(await readJsonBody(request)),
                projectId,
        ...
        if (request.method === "POST" && segments.length === 4 && segments[3] === "firewall-rules") {
            ...
            sendJson(response, 201, await repository.createFirewallRule({
                ...(await readJsonBody(request)),
                projectId
            }), options.allowedOrigin);
            return;
        ...
        if (request.method === "DELETE" && segments.length === 5 && segments[3] === "firewall-rules") {
            ...
            sendJson(response, 200, await repository.disableFirewallRule({
                ...(await readJsonBody(request)),
                projectId,
        ...
        if (request.method === "PUT" && segments.length === 5 && segments[3] === "edge-config") {
            ...
            sendJson(response, 200, await repository.upsertEdgeConfig({
                ...(await readJsonBody(request)),
                projectId,
        ...
        if (request.method === "DELETE" && segments.length === 5 && segments[3] === "edge-config") {
            ...
            sendJson(response, 200, await repository.deleteEdgeConfig({
                ...(await readJsonBody(request)),
                projectId,
                key: decodeURIComponent(segments[4])
            }), options.allowedOrigin);
        ...
        if (segments[3] === "blobs") {
            if ((request.method === "GET" || request.method === "HEAD") && segments.length === 4) {
                ...
                sendJson(response, 200, await repository.listBlobs({
                    projectId,
            ...
            if (request.method === "POST" && segments.length === 4) {
                ...
                sendJson(response, 201, await repository.putBlob({
                    ...(await readJsonBody(request)),
                    projectId
                }), options.allowedOrigin);
                return;
            ...
            if (segments.length >= 5) {
                ...
                if (request.method === "GET" || request.method === "HEAD") {
                    ...
                    sendJson(response, 200, await repository.getBlob({
                        projectId,
                        pathname
                    }), options.allowedOrigin, request.method);
                    return;
                ...
                if (request.method === "DELETE") {
                    ...
                    sendJson(response, 200, await repository.deleteBlob({
                        ...(await readJsonBody(request)),
                        projectId,
                        pathname
                    }), options.allowedOrigin);
        ...
        if (segments[3] === "cache") {
            if ((request.method === "GET" || request.method === "HEAD") && segments.length === 4) {
                ...
                sendJson(response, 200, await repository.listCacheEntries({
                    projectId,
            ...
            if (request.method === "POST" && segments.length === 5 && segments[4] === "purge") {
                ...
                sendJson(response, 200, await repository.purgeCache({
                    ...(await readJsonBody(request)),
                    projectId
                }), options.allowedOrigin);
                return;
        ...
        if (segments[3] === "functions") {
            if ((request.method === "GET" || request.method === "HEAD") && segments.length === 4) {
                ...
                sendJson(response, 200, await repository.listFunctions({
                    projectId,
                    deploymentId: url.searchParams.get("deploymentId") ?? undefined
                }), options.allowedOrigin, request.method);
                return;
            ...
            if ((request.method === "GET" || request.method === "HEAD") && segments.length >= 5) {
                ...
                sendJson(response, 200, await repository.getFunctionRuntime({
                    projectId,
        ...
        if (segments[3] === "routing-rules") {
            if ((request.method === "GET" || request.method === "HEAD") && segments.length === 4) {
                ...
                sendJson(response, 200, await repository.listRoutingRules({
                    projectId,
            ...
            if ((request.method === "GET" || request.method === "HEAD") && segments.length === 5 && segments[4] === "match") {
                ...
                sendJson(response, 200, await repository.matchRoutingRules({
                    projectId,
            ...
            if (request.method === "PUT" && segments.length === 4) {
                ...
                sendJson(response, 200, await repository.upsertRoutingRule({
                    ...(await readJsonBody(request)),
                    projectId
                }), options.allowedOrigin);
                return;
            ...
            if (request.method === "DELETE" && segments.length === 5) {
                ...
                sendJson(response, 200, await repository.disableRoutingRule({
                    ...(await readJsonBody(request)),
                    projectId,
                    ruleId: decodeURIComponent(segments[4])
                }), options.allowedOrigin);
        ...
        if (request.method === "DELETE" && segments.length === 5 && segments[3] === "deploy-hooks") {
            ...
            sendJson(response, 200, await repository.revokeDeployHook({
                ...(await readJsonBody(request)),
                projectId,
        ...
        if (request.method === "POST" && segments.length === 4 && segments[3] === "cron-jobs") {
            ...
            sendJson(response, 201, await repository.createCronJob({
                ...(await readJsonBody(request)),
                projectId
            }), options.allowedOrigin);
            return;
        ...
        if (request.method === "DELETE" && segments.length === 5 && segments[3] === "cron-jobs") {
            ...
            sendJson(response, 200, await repository.disableCronJob({
                ...(await readJsonBody(request)),
                projectId,
        ...
        if (request.method === "POST" && segments.length === 6 && segments[3] === "cron-jobs" && segments[5] === "run") {
            ...
            sendJson(response, 202, await repository.runCronJob({
                ...(await readJsonBody(request)),
                projectId,
                jobId: decodeURIComponent(segments[4])
            }), options.allowedOrigin);
        ...
        if (segments[3] === "rolling" && segments[4]) {
            ...
            if (request.method === "POST" && segments.length === 6 && segments[5] === "start") {
                ...
                sendJson(response, 202, await repository.startRollingRelease({
                    ...(await readJsonBody(request)),
                    projectId,
                    channel
                }), options.allowedOrigin);
            ...
            if (request.method === "POST" && segments.length === 6 && segments[5] === "advance") {
                ...
                sendJson(response, 202, await repository.advanceRollingRelease({
                    ...(await readJsonBody(request)),
                    projectId,
                    channel
                }), options.allowedOrigin);
            ...
            if (request.method === "POST" && segments.length === 6 && segments[5] === "complete") {
                ...
                sendJson(response, 202, await repository.completeRollingRelease({
                    ...(await readJsonBody(request)),
                    projectId,
                    channel
                }), options.allowedOrigin);
            ...
            if (request.method === "POST" && segments.length === 6 && segments[5] === "abort") {
                ...
                sendJson(response, 202, await repository.abortRollingRelease({
                    ...(await readJsonBody(request)),
                    projectId,
                    channel
                }), options.allowedOrigin);
        ...
        if ((segments[3] === "release" || segments[3] === "rollback") && segments[4]) {
            ...
            if (request.method === "POST" && segments.length === 6 && segments[3] === "release" && segments[5] === "promote") {
                ...
                sendJson(response, 202, await repository.promoteDeployment({
                    ...(await readJsonBody(request)),
                    projectId,
                    channel
                }), options.allowedOrigin);
            ...
            if (request.method === "POST" && segments.length === 6 && segments[3] === "rollback" && segments[5] === "rollback") {
                ...
                sendJson(response, 202, await repository.rollbackDeployment({
                    ...(await readJsonBody(request)),
                    projectId,
                    channel
                }), options.allowedOrigin);
    ...
    if (request.method === "POST" && segments.length === 3 && segments[1] === "deployments" && segments[2] === "prebuilt") {
        ...
        sendJson(response, 201, await repository.deployPrebuilt((await readJsonBody(request))), options.allowedOrigin);
        return;
    }
...
export function createSiteFlowServer(options) {
