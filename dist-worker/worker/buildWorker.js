...
import { publishBuildArtifact } from "./artifactPublisher.js";
...
function gitDeploymentConfig(value) {
    if (!isRecord(value)) {
        return undefined;
    }
    ...
    const rules = Object.entries(deploymentEnabled).flatMap(([pattern, enabled]) => {
...
async function runCommand(command, cwd, appendLog, environmentVariables, secretPatterns) {
...
function routeChannelFor(sourceEvent, productionBranch) {
    return sourceEvent.branch === productionBranch ? "production" : "preview";
}
function previewHostFor(job, deploymentId, baseDomain) {
...
function resolveProjectRoot(sourceDirectory, rootDirectory) {
...
function resolveOutputDirectory(projectRoot, outputDirectory) {
...
function functionRoutePath(relativePath) {
...
function functionConfigPatternMatches(routePath, sourcePath, pattern) {
...
function functionArtifactSourcePath(entry) {
    return entry.artifactPath.replace(/^\.siteflow\/functions\//, "");
}
function applyFunctionConfig(entries, config) {
...
function gitDeploymentEnabled(config, branch) {
    if (!config) {
        return true;
    }
...
async function collectIncludedFilesFromRoot(projectRoot, current, allowNodeModules, files) {
...
async function collectFunctionIncludeFiles(projectRoot, includePatterns, excludePatterns, occupiedArtifactPaths) {
...
async function collectFunctionEntrypoints(apiRoot, current, entries) {
...
async function detectFunctionEntrypoints(projectRoot) {
...
async function functionBundleExtraFiles(projectRoot, functions, configs) {
    const extraFiles = [];
...
export async function executeBuildJob(job, options) {
...
export async function runBuildWorkerOnce(options) {
...
export async function cleanupWorkspace(pathToRemove) {
