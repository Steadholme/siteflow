...
function defaultRepository(row) {
    return {
        provider: "generic",
        owner: "local",
...
function queueJobFromRow(row) {
    const repository = Object.keys(row.repository).length > 0 ? row.repository : defaultRepository(row);
    const projectBuildSettings = row.project_build_settings;
...
async function upsertBuildCronJob(client, projectId, cron) {
...
export class PostgresBuildQueue {
    pool;
    constructor(pool) {
        this.pool = pool;
    }
