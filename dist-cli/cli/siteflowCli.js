...
import { applyInstallPlan } from "./installApply.js";
...
function formatDeploymentsList(list) {
...
function formatDeploymentInspect(detail) {
...
function formatDeployHooksList(list) {
...
function formatDeployHookCreate(result) {
    return [
        `Status: ${result.status}`,
        `Hook:   ${result.hook.id}`,
...
function formatDeployHookRevoke(result) {
    return [
        `Status: ${result.status}`,
        `Hook:   ${result.hook.id}`,
        `Name:   ${result.hook.name}`,
...
function formatCronJobsList(list) {
...
function formatCronJobCreate(result) {
    return [
        `Status:   ${result.status}`,
        `Job:      ${result.job.id}`,
...
function formatCronJobDisable(result) {
    return [
        `Status: ${result.status}`,
        `Job:    ${result.job.id}`,
        `Name:   ${result.job.name}`,
...
function formatCronJobRun(result) {
    return [
        `Status:   ${result.status}`,
        `Message:  ${result.message}`,
...
function formatLogQuery(result) {
...
function formatLogDrainsList(list) {
...
function formatLogDrainCreate(result) {
    return [
        `Status:   ${result.status}`,
...
function formatLogDrainDelivery(result) {
    return [
        `Status:   ${result.status}`,
        `Message:  ${result.message}`,
        `Drain:    ${result.drain.id}`,
...
function formatAuditEvents(settings) {
...
function formatApiTokens(settings) {
...
function formatApiTokenCreate(result) {
    return [
        `Status: ${result.status}`,
...
function formatApiTokenRevoke(result) {
    return [
        `Status: ${result.status}`,
        `Token:  ${result.token.id}`,
        `Name:   ${result.token.name}`,
...
function formatFirewallConditions(rule) {
    const parts = [
...
function formatFirewallRules(list) {
...
function formatFirewallRuleMutation(result) {
    return [
        `Status:     ${result.status}`,
        `Rule:       ${result.rule.id}`,
...
function formatEdgeConfig(list) {
...
function formatEdgeConfigMutation(result) {
    return [
        `Status: ${result.status}`,
...
function formatBlobMutation(result) {
    return [
        `Status:       ${result.status}`,
...
function formatBlobRead(result, output) {
    return [
        "Status:       downloaded",
        `Pathname:     ${result.blob.pathname}`,
        `Content-Type: ${result.blob.contentType}`,
...
function formatCacheEntries(list) {
    if (list.entries.length === 0) {
...
function formatCachePurge(result) {
    return [
        `Status: ${result.status}`,
        `Purged: ${result.total}`,
...
function formatFunctionRuntime(result) {
...
function formatRoutingRuleMutation(result) {
    return [
        `Status:   ${result.status}`,
        `Rule:     ${result.rule.id}`,
        `Name:     ${result.rule.name}`,
...
function formatRollingCommandResult(result) {
...
function idempotencyKey(prefix, deploymentId, channel) {
    return `${prefix}:${deploymentId}:${channel}`;
}
function flagStringList(flags, name) {
...
function headerCondition(value) {
    if (!value) {
        return undefined;
    }
...
function optionalIntegerFlag(parsed, name) {
...
function rollingReason(parsed, action, channel, env, deploymentId) {
    const target = deploymentId ? ` ${deploymentId}` : "";
    return flagString(parsed.flags, "reason")
        ?? env.SITEFLOW_RELEASE_REASON
        ?? `${action} rolling release${target} on ${channel} via SiteFlow CLI.`;
}
function requiredPercentage(parsed) {
...
async function resolveApiSession(parsed, env) {
...
async function resolveLinkedProject(parsed, env) {
...
async function fetchApiJson(serverUrl, pathName, apiToken, fetchImpl, init = {}) {
    const headers = {
...
async function runLink(parsed, io, env, fetchImpl) {
    ...
    try {
        ...
        const settings = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(projectId)}/settings`, session.apiToken, fetchImpl);
        const link = {
...
function envSkeleton(settings, targetEnvironment) {
...
async function runEnvPull(parsed, io, env, fetchImpl) {
    const json = flagBoolean(parsed.flags, "json");
    ...
    try {
        ...
        const settings = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/settings`, session.apiToken, fetchImpl);
...
async function runReleaseCommand(action, parsed, io, env, fetchImpl) {
    const json = flagBoolean(parsed.flags, "json");
    ...
    try {
        ...
        const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/${action === "promote" ? "release" : "rollback"}/${encodeURIComponent(channel)}/${action}`, session.apiToken, fetchImpl, {
            method: "POST",
...
async function runDeployHook(parsed, io, env, fetchImpl) {
    const json = flagBoolean(parsed.flags, "json");
    ...
    try {
        ...
        if (action === "list") {
            const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/deploy-hooks`, session.apiToken, fetchImpl);
            if (json) {
                writeJson(io, result);
            }
            else {
        ...
        const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/deploy-hooks/${encodeURIComponent(hookId)}`, session.apiToken, fetchImpl, {
            method: "DELETE",
            body: JSON.stringify({
                projectId: linked.projectId,
                hookId,
...
async function runCronCommand(parsed, io, env, fetchImpl) {
    const json = flagBoolean(parsed.flags, "json");
    const action = parsed.positionals[0];
    ...
    try {
        ...
        if (action === "list") {
            const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/cron-jobs`, session.apiToken, fetchImpl);
            if (json) {
                writeJson(io, result);
            }
            else {
        ...
        if (action === "disable") {
            const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/cron-jobs/${encodeURIComponent(jobId)}`, session.apiToken, fetchImpl, {
                method: "DELETE",
                body: JSON.stringify({
                    projectId: linked.projectId,
                    jobId,
        ...
        const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/cron-jobs/${encodeURIComponent(jobId)}/run`, session.apiToken, fetchImpl, {
            method: "POST",
            body: JSON.stringify({
                projectId: linked.projectId,
                jobId,
...
async function runLogsCommand(parsed, io, env, fetchImpl) {
    const json = flagBoolean(parsed.flags, "json");
    try {
        const linked = await resolveLinkedProject(parsed, env);
        const session = await resolveApiSession({
        ...
        const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/logs${query ? `?${query}` : ""}`, session.apiToken, fetchImpl);
        if (json) {
            writeJson(io, result);
        }
        else {
...
async function runLogDrainCommand(parsed, io, env, fetchImpl) {
    const json = flagBoolean(parsed.flags, "json");
    const action = parsed.positionals[0];
    ...
    try {
        ...
        if (action === "list") {
            const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/log-drains`, session.apiToken, fetchImpl);
            if (json) {
                writeJson(io, result);
            }
            else {
        ...
        const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/log-drains/${encodeURIComponent(drainId)}/deliver`, session.apiToken, fetchImpl, {
            method: "POST",
            body: JSON.stringify({
                projectId: linked.projectId,
                drainId,
...
async function readProjectSettingsForCli(parsed, env, fetchImpl) {
    const linked = await resolveLinkedProject(parsed, env);
    const session = await resolveApiSession({
        ...parsed,
        flags: {
...
async function runAuditCommand(parsed, io, env, fetchImpl) {
    const json = flagBoolean(parsed.flags, "json");
...
async function runApiTokenCommand(parsed, io, env, fetchImpl) {
    const json = flagBoolean(parsed.flags, "json");
    const action = parsed.positionals[0];
    if (action !== "create" && action !== "list" && action !== "revoke") {
    ...
    try {
        ...
        if (action === "create") {
            ...
            const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/api-tokens`, session.apiToken, fetchImpl, {
                method: "POST",
                body: JSON.stringify({
                    projectId: linked.projectId,
                    name,
        ...
        const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/api-tokens/${encodeURIComponent(tokenId)}`, session.apiToken, fetchImpl, {
            method: "DELETE",
            body: JSON.stringify({
                projectId: linked.projectId,
                tokenId,
...
async function runFirewallCommand(parsed, io, env, fetchImpl) {
    const json = flagBoolean(parsed.flags, "json");
    const action = parsed.positionals[0];
    ...
    try {
        ...
        if (action === "list") {
            const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/firewall-rules`, session.apiToken, fetchImpl);
            if (json) {
                writeJson(io, result);
            }
            else {
        ...
        const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/firewall-rules/${encodeURIComponent(ruleId)}`, session.apiToken, fetchImpl, {
            method: "DELETE",
            body: JSON.stringify({
                projectId: linked.projectId,
                ruleId,
...
async function runEdgeConfigCommand(parsed, io, env, fetchImpl) {
    const json = flagBoolean(parsed.flags, "json");
    const action = parsed.positionals[0];
    ...
    try {
        ...
        if (action === "list") {
            const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/edge-config`, session.apiToken, fetchImpl);
            if (json) {
                writeJson(io, result);
            }
            else {
        ...
        if (action === "set") {
            ...
            const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/edge-config/${encodeURIComponent(key)}`, session.apiToken, fetchImpl, {
                method: "PUT",
                body: JSON.stringify({
                    projectId: linked.projectId,
                    key,
...
async function runBlobCommand(parsed, io, env, fetchImpl) {
    const json = flagBoolean(parsed.flags, "json");
    const action = parsed.positionals[0];
    ...
    try {
        ...
        if (action === "list") {
            ...
            const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/blobs${queryString ? `?${queryString}` : ""}`, session.apiToken, fetchImpl);
            if (json) {
                writeJson(io, result);
            }
            else {
        ...
        if (action === "put") {
            ...
            const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/blobs`, session.apiToken, fetchImpl, {
                method: "POST",
                body: JSON.stringify({
                    projectId: linked.projectId,
        ...
        if (action === "get") {
            ...
            const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/blobs/${encodeURIComponent(pathname)}`, session.apiToken, fetchImpl);
...
async function runCacheCommand(parsed, io, env, fetchImpl) {
    const json = flagBoolean(parsed.flags, "json");
    const action = parsed.positionals[0];
    ...
    try {
        ...
        if (action === "list") {
            ...
            const queryString = query.toString();
            const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/cache${queryString ? `?${queryString}` : ""}`, session.apiToken, fetchImpl);
            if (json) {
                writeJson(io, result);
            }
            else {
        ...
        const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/cache/purge`, session.apiToken, fetchImpl, {
            method: "POST",
            body: JSON.stringify({
                projectId: linked.projectId,
                path: pathName,
...
async function runFunctionsCommand(parsed, io, env, fetchImpl) {
    const json = flagBoolean(parsed.flags, "json");
    const action = parsed.positionals[0];
    ...
    try {
        ...
        if (action === "list") {
            ...
            const queryString = query.toString();
            const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/functions${queryString ? `?${queryString}` : ""}`, session.apiToken, fetchImpl);
            if (json) {
                writeJson(io, result);
            }
            else {
        ...
        const queryString = query.toString();
        const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/functions/${encodeURIComponent(functionPath)}${queryString ? `?${queryString}` : ""}`, session.apiToken, fetchImpl);
        if (json) {
            writeJson(io, result);
        }
        else {
...
async function runRoutingRulesCommand(parsed, io, env, fetchImpl) {
    const json = flagBoolean(parsed.flags, "json");
    const action = parsed.positionals[0];
    ...
    try {
        ...
        if (action === "list") {
            ...
            const queryString = query.toString();
            const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/routing-rules${queryString ? `?${queryString}` : ""}`, session.apiToken, fetchImpl);
            if (json) {
                writeJson(io, result);
            }
            else {
        ...
        if (action === "upsert") {
            ...
            const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/routing-rules`, session.apiToken, fetchImpl, {
                method: "PUT",
                body: JSON.stringify({
                    projectId: linked.projectId,
                    name,
        ...
        const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/routing-rules/${encodeURIComponent(ruleId)}`, session.apiToken, fetchImpl, {
            method: "DELETE",
            body: JSON.stringify({
                projectId: linked.projectId,
                ruleId,
...
async function runRollingRelease(parsed, io, env, fetchImpl) {
    const json = flagBoolean(parsed.flags, "json");
    const action = parsed.positionals[0];
    ...
    try {
        ...
        const result = await fetchApiJson(session.serverUrl, `/api/projects/${encodeURIComponent(linked.projectId)}/rolling/${encodeURIComponent(channel)}/${action}`, session.apiToken, fetchImpl, {
            method: "POST",
            body: JSON.stringify(body)
        });
        if (json) {
...
async function promoteDeployResult(parsed, env, fetchImpl, serverUrl, apiToken, deploymentId, projectId) {
    const channel = flagString(parsed.flags, "channel") ?? "production";
    const reason = commandReason(parsed, "promote", deploymentId, channel, env);
    return fetchApiJson(serverUrl, `/api/projects/${encodeURIComponent(projectId)}/release/${encodeURIComponent(channel)}/promote`, apiToken, fetchImpl, {
        method: "POST",
        body: JSON.stringify({
            projectId,
            channel,
...
async function runDeployments(parsed, io, env, fetchImpl) {
    const json = flagBoolean(parsed.flags, "json");
    const projectId = flagString(parsed.flags, "project-id") ?? flagString(parsed.flags, "project");
    try {
        const session = await resolveApiSession(parsed, env);
        ...
        const query = params.toString();
        const list = await fetchApiJson(session.serverUrl, `/api/deployments${query ? `?${query}` : ""}`, session.apiToken, fetchImpl);
        if (json) {
            writeJson(io, list);
        }
        else {
...
async function runInspect(parsed, io, env, fetchImpl) {
    const json = flagBoolean(parsed.flags, "json");
    const deploymentId = parsed.positionals[0];
    if (!deploymentId) {
    ...
    try {
        const session = await resolveApiSession(parsed, env);
        const detail = await fetchApiJson(session.serverUrl, `/api/deployments/${encodeURIComponent(deploymentId)}`, session.apiToken, fetchImpl);
        if (json) {
            writeJson(io, detail);
        }
        else {
...
async function verifyLogin(serverUrl, token, fetchImpl) {
