...
function renderedAsset(path, content) {
    return {
        path,
...
export function normalizeDnsName(value, label = "Domain") {
...
export function renderManagedNginxConfig(options) {
    ...
    if (controlPlaneHost) {
        blocks.push("", "server {", "    listen 80;", `    server_name ${controlPlaneHost};`, "", `    client_max_body_size ${clientMaxBodySize};`, "", proxyLocation(), "}");
    }
    if (wildcardBaseDomain) {
        blocks.push("", "server {", "    listen 80;", `    server_name *.${wildcardBaseDomain};`, "", `    client_max_body_size ${clientMaxBodySize};`, "", "    location = /api {", "        return 404;", "    }", "", "    location ^~ /api/ {", "        return 404;", "    }", "", "    location = /healthz {", "        return 404;", "    }", "", proxyLocation(), "}");
    }
...
export function renderSiteFlowEnvFile(options) {
    const lines = [
...
export function renderComposeFile(options) {
