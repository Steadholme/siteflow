...
- The workflow now inspects the published image in the registry and records SLSA provenance and SBOM attestation manifest plus statement metadata in that evidence artifact.
- The final release evidence bundle now requires that `release-image-evidence.json` artifact and checks its schema, GHCR digest, image tags, source repository/commit, GitHub run metadata, registry attestation subjects, provenance/SBOM predicate types, statement digests, provenance source identity, builder/material evidence, explicit failedChecks, inspection freshness, and evidence freshness before `release:evidence` can pass.
...
- Installer-generated Compose explicitly sets `SITEFLOW_BUILD_RUNNER=docker`, runs the worker as non-root `SITEFLOW_WORKER_USER=1000:1000` by default, requires `SITEFLOW_DOCKER_SOCKET_GID` to match the target host Docker socket group, mounts the host Docker socket, and checks `command -v docker` plus `docker info` before starting the worker so missing Docker CLI/daemon access fails closed.
...
- A clean gap report is not production evidence. Production promotion still requires every target command to run and `npm run release:evidence -- --evidence <release-evidence.json> --commit-ref <sha> --repo <owner/repo> --branch main --target-environment production --attestation-key-env SITEFLOW_RELEASE_EVIDENCE_SIGNING_KEY --json` to pass for the exact release commit.
...
- The CLI validates the supplied bundle with the same `release:evidence` checker semantics for `targetEnvironment=production`; current production promotion paths send a `{ evidencePath, bundle }` envelope so the server can verify the same full bundle before storing normalized metadata.
...
- `siteflow rolling start|advance|complete --channel production` now uses the same CLI release evidence gate as production promote/deploy and sends a full release evidence bundle envelope to the API. `rolling abort` remains available without a full release evidence bundle because it stops an in-progress rollout, but production abort now requires an explicit audit reason and records a stop-rollout release evidence exception.
- The HTTP API now rejects direct production `release/production/promote` calls and production rolling `start`, `advance`, or `complete` calls unless the request body contains a full production release evidence bundle envelope that can be evaluated and normalized.
- Non-production promotion and rolling commands remain compatible without release evidence metadata.
- This closes the local CLI and direct HTTP empty-evidence bypasses. Full production promotion still requires a passing signed bundle for the exact commit and target environment, protected GitHub branch/commit evidence, and target runtime evidence.
...
### Wave 081 - release evidence pack contract CI and signing key trust

- Added `npm run release:evidence:pack-contract -- --json` as an offline CI gate. It generates a representative rehearsal pack, validates the shared pack contract, checks expected operator/env placeholders, runs the `release:evidence:target-run --plan-only` path with fake local `npm` / `gh` executables, and scans the generated result for sensitive-looking output without touching target systems.
- The rehearsal pack contract now requires the final compose and check commands to include `--attestation-key-env SITEFLOW_RELEASE_EVIDENCE_SIGNING_KEY`, `--attestation-key-id-env SITEFLOW_RELEASE_EVIDENCE_REQUIRED_SIGNING_KEY_ID`, and the matching command `env` requirements.
- The CLI production release evidence gate now resolves `SITEFLOW_RELEASE_EVIDENCE_SIGNING_KEY` or `SITEFLOW_RELEASE_EVIDENCE_SIGNING_KEY_FILE` and passes the key into the real bundle evaluator before production promote/deploy/rollback/rolling requests are sent.
- The HTTP server can pin a required non-secret attestation key id with `SITEFLOW_RELEASE_EVIDENCE_REQUIRED_SIGNING_KEY_ID` or `SITEFLOW_RELEASE_EVIDENCE_SIGNING_KEY_ID`; mismatched signed bundles are rejected before the evaluator or repository mutation runs.
- This hardens the release evidence enforcement chain, but it still does not create target evidence. Production remains blocked until the exact release commit has a passing signed bundle from real target or target-equivalent systems.
...
### Wave 100 - local production gate tightening

- `/metrics` now includes local artifact, release evidence, and temp free-byte gauges plus storage missing-path and collection-error gauges. The provisioning plan renders storage alerts and Grafana panels, and observability evidence requires the storage metric names in the scrape.
- Release evidence bundle checks, release gap reports, and the CLI production evidence gate now default the required attestation key id from `SITEFLOW_RELEASE_EVIDENCE_REQUIRED_SIGNING_KEY_ID` or legacy `SITEFLOW_RELEASE_EVIDENCE_SIGNING_KEY_ID`, so a valid signature from the wrong pinned key id is blocked before mutation.
- Post-promotion production evidence now requires fresh structured probes: `/readyz` must return `200`, `/metrics` must return `200`, `401`, or `403`, and both probe files must carry a fresh ISO timestamp unless a ticketed, approved, time-bounded production exception is supplied.
- Target runtime evidence now requires Compose healthcheck summaries for Postgres, API, and worker.
- CI, release-preflight, and release-image workflows now run `release:commit:plan -- --fail-on-blocked --json` and `release:evidence:pack-contract -- --json`, and `siteflow release-gate` statically checks release-image workflow gates plus stricter production Compose worker command, worker healthcheck, API loopback bind, no-build, and service-scope constraints.
- These changes reduce local false-greens. They still do not create target Docker, Postgres, source-provider, backup, observability, ingress, credential, or rollback evidence.

### Wave 101 - source provider collector and image provenance contract alignment

- Added `npm run source-provider:evidence:collect` for GitHub source-provider evidence. The collector reads repository, branch, and deploy-key metadata, writes raw source-provider evidence, runs the same checker to write the release-bundle input, and avoids archiving token values, authorization headers, webhook secrets, signature headers, private keys, or raw host keys.
- The collector stays fail-closed: missing target-side signed webhook proof, exact commit binding, deploy-key proof for private repositories, or SSH host-key proof writes blocked evidence instead of a passing artifact.
- Release rehearsal packs and release-preflight target runs now carry explicit source-provider placeholder replacements for webhook delivery id, deploy-key path, and known-hosts path. The pack contract, target-run tests, and release gate static workflow checks enforce those terms.
- Release image passed fixtures now match the stricter provenance/SBOM contract: registry-inspected subject digests, statement digests, provenance source identity, builder, and materials are required before the final release evidence bundle can pass.
- This improves evidence collection and false-green resistance. It still does not prove a real target provider, branch protection, exact CI status, target runtime, backup, observability, ingress, credential, or rollback posture.

### Wave 102 - release commit readiness generated policy alignment

- `release:commit:plan` now derives `excludedFromStaging` from the same release source policy used by `release:source:check`, including `evidence/`, `.vite/`, `release-commit-readiness-plan*.json`, `npm-debug.log*`, and the rest of the generated, secret, dependency, build, test-output, and scratch pathspecs.
- The readiness tests now assert ordinary non-forbidden untracked source files are reported as blocking `untrackedSource` findings while generated/forbidden paths stay excluded, and they check that `.gitignore` and `.dockerignore` carry the generated release pathspecs.
- The plan now reports `stagingCoverage` so operators and CI logs can verify truncated finding lists still have complete explicit staging pathspec coverage and no generated, secret, evidence, dependency, build, test-output, or scratch pathspecs were suggested.
- The CLI remains read-only and advisory by default; `--fail-on-blocked` is still the opt-in path that returns exit code `1` when blocked findings are present.

## CI and release surface

- `.github/workflows/ci.yml` is the current minimum CI gate. It checks dependency policy, release source policy, commit readiness, release evidence pack contract, production Compose config parsing, dependency installation, unit/integration tests, the production build command, release artifact evidence, and Playwright E2E safeguards.
- `.github/workflows/release-preflight.yml` is a manual preflight workflow for release operators. It is not a deployment workflow and does not replace target evidence, but it runs the same commit-readiness and pack-contract gates, archives promotion release-gate evidence, the release evidence rehearsal pack, the target-run/gap reports, and Playwright artifacts for the exact release commit.
- `.github/workflows/release-image.yml` publishes runtime images only after source, commit-readiness, pack-contract, test, build, artifact, registry provenance/SBOM, and release-image evidence gates pass.
...
- `npm run release:commit:plan -- --fail-on-blocked` can be used in CI or release-preflight automation to make dirty/untracked release-readiness state fail by exit code. Without that flag it remains a read-only advisory report for local planning; review `stagingCoverage` before staging to confirm the suggested pathspecs cover every required dirty/untracked source path and exclude generated or forbidden release-source paths. Use `--review-checklist` for the human Markdown pathspec review; when combining it with `--json`, write it through `--review-checklist-output` so stdout remains parseable JSON, and place any JSON or Markdown output under a temp or release evidence directory rather than the repository root.
...
- Generate a target release evidence rehearsal pack before the promotion window with `npm run --silent release:evidence:rehearsal-pack -- --commit-ref <sha> --repo <owner/repo> --branch main --target-env-file <target-env-file> --public-base-url <https-url> --operator-name <operator> --release-ticket <ticket> --docker-socket-profile-accepted --output-dir <evidence-dir>`. Pass `--docker-socket-profile-accepted` only after the release owner records acceptance of the trusted single-host Docker socket worker profile. Use the generated Markdown as the operator checklist for the real evidence run; source provider, operator access, and non-session credential steps include collector commands that write raw evidence plus checker output, while matching template commands remain only as non-passing fallback skeletons.
- During the evidence run, use `npm run --silent release:evidence:target-run -- --pack <evidence-dir>/release-evidence-rehearsal-pack.json --confirm-target-environment production --set direct-api-url=<direct-api-health-url> --set release-image-digest=sha256:<release-image-digest> --set release-image-run-id=<github-actions-run-id> --set webhook-delivery-id=<source-provider-webhook-delivery-id> --set deploy-key-path=<target-deploy-key-path> --set known-hosts-path=<target-known-hosts-path> --set SITEFLOW_TRUST_PROXY=<trust-proxy-policy> --set api-instance-count=<count> --set api-process-count=<count> --set ingress-count=<count> --set api-rate-limit-scope=<edge|shared|global|distributed|process_local> --set api-rate-limit-enforcement-point=<edge|proxy|load_balancer|gateway|ingress|cdn|api> --set operator-access-project-id=<allowed-project-id> --set operator-access-denied-project-id=<denied-project-id> --set old-metrics-token-redacted-id=<old-metrics-token-redacted-id> --set new-metrics-token-redacted-id=<new-metrics-token-redacted-id> --set old-root-api-token-redacted-id=<old-root-api-token-redacted-id> --set new-root-api-token-redacted-id=<new-root-api-token-redacted-id> --set break-glass-source=<vault-or-ticket-system> --set break-glass-approver-count=<count> --json` to execute pack commands and archive gap snapshots, or add `--plan-only` to the same target-run command first to validate placeholders, required environment variable names, and command executability without running target commands or producing production evidence. Use `npm run --silent release:evidence:gaps -- --pack <evidence-dir>/release-evidence-rehearsal-pack.json` with the same `--set` or `--set-env` replacements as a read-only placeholder preflight. These commands first validate that the pack command semantics still match the generated contract, and none stores or prints `--set` replacement values. Before final bundle composition, the gap report is expected to show missing final bundle and final check outputs while helping operators clear target evidence outputs and immediate input gaps. After compose and final check complete, the final gap report or target-run final snapshot must have no missing, stale, manual-required, dry-run-only, failed, identity-mismatched, blocked, or invalid items. Neither target-run, target-run plan-only, nor gaps is a promotion gate by itself.
- After collecting promotion, Docker build rehearsal, Postgres rehearsal, release artifact, release image, target runtime, source provider, backup, observability, operator access, non-session credential, ingress, and upgrade/rollback drill outputs, set `SITEFLOW_RELEASE_EVIDENCE_SIGNING_KEY` or `SITEFLOW_RELEASE_EVIDENCE_SIGNING_KEY_FILE` and `SITEFLOW_RELEASE_EVIDENCE_REQUIRED_SIGNING_KEY_ID`, run `npm run --silent release:evidence:compose -- --release-gate <release-gate.json> --docker-build <docker-build.json> --postgres-rehearsal <postgres.json> --artifact-evidence <release-artifact-evidence.json> --release-image-evidence <release-image-evidence.json> --target-runtime-evidence <target-runtime-evidence.json> --source-provider-evidence <source-provider-evidence.json> --backup-evidence <backup.json> --observability-evidence <observability.json> --operator-access-evidence <operator-access.json> --non-session-credential-evidence <non-session-credential.json> --ingress-evidence <ingress.json> --upgrade-rollback-evidence <upgrade-rollback.json> --target-environment production --operator-name <operator> --release-ticket <ticket> --docker-socket-profile-accepted --attestation-key-env SITEFLOW_RELEASE_EVIDENCE_SIGNING_KEY --attestation-key-id-env SITEFLOW_RELEASE_EVIDENCE_REQUIRED_SIGNING_KEY_ID --output <release-evidence.json>`, then run `npm run --silent release:evidence -- --evidence <release-evidence.json> --commit-ref <sha> --repo <owner/repo> --branch main --target-environment production --attestation-key-env SITEFLOW_RELEASE_EVIDENCE_SIGNING_KEY --attestation-key-id-env SITEFLOW_RELEASE_EVIDENCE_REQUIRED_SIGNING_KEY_ID --json` as the combined promotion evidence check. The raw release-gate JSON must include fresh `checkedAt` values from `siteflow release-gate --promotion --json`; repackaging old release-gate output under a new bundle wrapper is rejected. The generated pack includes `--docker-socket-profile-accepted` only when the pack generator was invoked with that explicit flag. Use `--host-build-exception-accepted` only when the release gate records a deliberate trusted-source host-build exception. Mutating CLI production promotion commands, including production rolling `start`, `advance`, and `complete`, must include the same bundle path with `--release-evidence <release-evidence.json>`; the server verifies the bundle attestation signature and re-runs the release evidence checker before it stores normalized metadata.
...
- Installer-generated and committed production Compose profiles mount `/etc/siteflow/secrets/app-secret.secret` as a Docker secret and pass it as `SITEFLOW_APP_SECRET_FILE`; they do not `cat` or `export` raw secret values in service commands.
...
- Compose passes `SITEFLOW_APP_SECRET_FILE` to API and worker processes before launching `dist-server/server/index.js` and `dist-worker/worker/index.js`; the services resolve the file internally and raw secret values are not expanded into Compose commands.
...
- CI already covers source policy, commit-readiness planning, release evidence pack contract checks, production Compose config parsing, release artifact gates, tests, build, and E2E. It should expand further to dependency/security scanning and deeper image provenance/security checks.
...
- GitHub Actions CI is green for dependency policy, release source policy, commit readiness, release evidence pack contract, production Compose config parsing, `npm ci`, `npm test -- --run`, `npm run build`, release artifact checks, and `npm run test:e2e`.
...
- `npm run --silent operator-access:evidence:collect -- --base-url <target-url> --commit-ref <sha> --repo <owner/repo> --branch main --target-environment production --operator-name <operator> --release-ticket <ticket> --project-id <allowed-project-id> --denied-project-id <denied-project-id> --admin-token-env SITEFLOW_API_TOKEN --low-scope-token-env SITEFLOW_OPERATOR_LOW_SCOPE_TOKEN --execute-project-cutoff --execute-global-cutoff --i-understand-this-revokes-active-operator-sessions --browser-token-fallback-disabled --local-storage-fallback-disabled --output <operator-access-evidence-raw.json> --check-output <operator-access-evidence.json> --json` has produced passed checker output for non-dry-run target or target-equivalent evidence covering disabled browser token fallback posture, disabled `localStorage` token fallback, operator session creation and rotation, Secure HttpOnly cookie flags, secret-free responses, idle timeout, project scope, CSRF, Bearer precedence, server-derived actor attribution, current-session revoke, global/project emergency cutoff, old-cookie rejection, and no raw credential archival. This collector creates sessions, creates and disables a temporary routing rule, and global cutoff revokes active operator sessions.
- `npm run --silent non-session-credential:evidence:collect -- --base-url <target-url> --old-metrics-token-env SITEFLOW_OLD_METRICS_TOKEN --new-metrics-token-env SITEFLOW_METRICS_TOKEN --old-api-token-env SITEFLOW_OLD_API_TOKEN --new-api-token-env SITEFLOW_API_TOKEN --old-redacted-identifier <old-metrics-token-redacted-id> --new-redacted-identifier <new-metrics-token-redacted-id> --old-api-redacted-identifier <old-root-api-token-redacted-id> --new-api-redacted-identifier <new-root-api-token-redacted-id> --commit-ref <sha> --repo <owner/repo> --branch main --target-environment production --operator-name <operator> --release-ticket <ticket> --break-glass-source <vault-or-ticket-system> --break-glass-approver-count <count> --break-glass-reviewed --break-glass-time-bounded --break-glass-revocation-planned --output <non-session-credential-evidence-raw.json> --check-output <non-session-credential-evidence.json> --json` has produced passed checker output for non-dry-run target or target-equivalent evidence covering metrics and root API token rotation or break-glass handling, redacted old/new identifiers, no raw credential archival, old credential rejection, new credential acceptance, scoped token audit/cutover, runtime token strength/reload, app-secret reseal/rollback planning, provider-managed rotation proof, break-glass controls, operator, and ticket metadata.
